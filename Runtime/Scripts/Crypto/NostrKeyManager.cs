    ...
    public class NostrKeyManager
    {
        ...
        public string GeneratePrivateKey(bool useHex = true)
        {
            try
            {
                ...
                if (useHex)
                {
                    return Hex.BytesToHex(privateKey);
                }
                else
                {
        ...
        /// <param name="useCompressed">Whether to return compressed format with prefix (true) or raw 64-char format (false)</param>
        /// <returns>The public key in requested format</returns>
        public string GetPublicKey(string privateKey, bool useCompressed = true)
        {
            try
            {
                ...
                string hexPrivateKey = privateKey.ToLowerInvariant();
                ...
                // Use Secp256k1Manager to derive the public key (always returns compressed format)
                byte[] privateKeyBytes = Hex.HexToBytes(hexPrivateKey);
                byte[] publicKeyBytes = Secp256k1Manager.GetPublicKey(privateKeyBytes);
                
                // The raw key format is what the Nostr protocol expects in events (no prefix)
                string publicKeyHex = Hex.BytesToHex(publicKeyBytes);
                
                if (useCompressed)
                {
                    // Secp256k1Manager.GetPublicKey already returns compressed format with prefix
                    return publicKeyHex;
                }
                else
                {
                    // Strip the compression prefix (02/03) for raw format
        ...
        public string SignMessage(string message, string privateKey)
        {
            try
            {
                ...
                return Hex.BytesToHex(signatureBytes);
            }
            catch (Exception ex)
            {
        ...
        public bool VerifySignature(string message, string signature, string publicKey)
        {
            try
            {
                ...
                byte[] signatureBytes = Hex.HexToBytes(signature);
                
                // Convert the public key to bytes
                byte[] publicKeyBytes = Hex.HexToBytes(hexPublicKey);
        ...
        /// <summary>
        /// Encrypts a key with AES using the given password
        /// </summary>
        private byte[] EncryptKey(string key, string password, byte[] iv)
        {
            using (var aes = Aes.Create())
            {
                aes.Key = GenerateKeyFromPassword(password);
                aes.IV = iv;
        ...
        /// <summary>
        /// Decrypts a key with AES using the given password
        /// </summary>
        ...
        /// <summary>
        /// Generates a 256-bit key from a password using SHA256
        /// </summary>
